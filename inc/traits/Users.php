...
trait Users {
    ...
    public function getMyComments($pageNo = null) {
        ...
        foreach ($comments as $comment) {
            ...
            $comment->posted_as = get_field("posted_as", "comment_" . $comment->comment_ID);
        }
    ...
    public function getMyEntities() {
        return $this->getResults(['post_type' => 'entities', 'posts_per_page' => -1, 'author' => $this->userId]);
    }

    public function checkFollow($id = null) {
        global $wpdb;
        $id = is_null($id) ? $_REQUEST['id'] : $id;
        ...
        return $wpdb->get_row("
                    SELECT * FROM " . $wpdb->prefix . "connections
                        WHERE 
                        follower = $this->userId and followee = $id
                ");
    }
